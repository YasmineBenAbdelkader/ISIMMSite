const PubAppelOffre = require('../Models/PubAppelOffre'); 
const moment = require('moment');

// Ajout par admin
exports.AjouterappelOffre = async (req, res) => {
    try {
        const appelOffreId = req.params.appelOffreId;

        // Create a new instance of PubAppelOffre with form data and the reference to AppelOffre
        const nouvellePubAppelOffre = new PubAppelOffre({
            titre: req.body.titre,
            description: req.body.description,
            piece_jointe: req.file ? req.file.filename : null,
        });

        const pubAppelOffreEnregistre = await nouvellePubAppelOffre.save();

        console.log('Publication ajoutée :', pubAppelOffreEnregistre);

        res.status(201).json({
            message: 'Publication ajoutée avec succès',
            pubAppelOffre: pubAppelOffreEnregistre,
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({
            message: `Une erreur s'est produite lors de l'ajout de la publication : ${error.message}`,
            error: error.stack,
        });
    }
};

// Suppression par admin
exports.SupprimerappelOffre = async (req, res) => {
    try {
        const appelOffreId = req.params.appelOffreId;

        // Vérifiez si l'ID de la publication est fourni
        if (!appelOffreId) {
            return res.status(400).json({
                message: "L'ID de la publication d'appel d'offres est requis pour la suppression.",
            });
        }

        // Recherchez la publication d'appel d'offres par ID et supprimez-la
        const pubAppelOffreSupprimee = await PubAppelOffre.findByIdAndDelete(appelOffreId);

        // Vérifiez si la publication a été trouvée et supprimée
        if (!pubAppelOffreSupprimee) {
            return res.status(404).json({
                message: "La publication d'appel d'offres avec cet ID n'a pas été trouvée.",
            });
        }

        console.log('Publication supprimée :', pubAppelOffreSupprimee);

        res.status(200).json({
            message: 'Publication supprimée avec succès',
            pubAppelOffre: pubAppelOffreSupprimee,
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({
            message: `Une erreur s'est produite lors de la suppression de la publication : ${error.message}`,
            error: error.stack,
        });
    }
};

// Afficher les 4 dernières publications avec leurs détails
exports.AfficherDernieresPublications = async (req, res) => {
    try {
        // Récupérez les 4 dernières publications en triant par ordre décroissant d'ID (ou un autre champ de date)
        const dernieresPublications = await PubAppelOffre.find().sort({ _id: -1 }).limit(4);

        // Formatez les dates au format "jj/mm/aaaa"
        const publicationsFormatees = dernieresPublications.map(publication => {
            return {
                _id: publication._id,
                titre: publication.titre,
                description: publication.description,
                piece_jointe: publication.piece_jointe,
                date_creation: moment(publication.date_creation).format('DD/MM/YYYY'),
            };
        });

        res.status(200).json({
            message: 'Les 4 dernières publications ont été récupérées avec succès',
            publications: publicationsFormatees,
        });
    } catch (error) {
        console.error(error);
        res.status(500).json({
            message: `Une erreur s'est produite lors de la récupération des dernières publications : ${error.message}`,
            error: error.stack,
        });
    }
};